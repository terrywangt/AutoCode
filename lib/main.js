...
function AutoCore(database, username, password, options) {
    ...
    this.options =Object.assign({
        ...
        tables:[],//只查询的table名称，未空则查询所有
        skipTables:[],//跳过查询table名称
    }, options || {});
}
...
AutoCore.prototype.build = function(callback) {
    ...
    if (self.options.dialect === 'postgres' && self.options.schema) {
        ...
        //this.queryInterface.describeTable('tb_bill_loan_info').then((a,b,c)=>console.log(a,b,c),()=>{})
        //this.queryInterface.showAllSchemas().then((a,b,c)=>console.log(a,b,c),()=>{})
...
AutoCore.prototype.writeFile = function(filename, directory,data, callback) {    
    mkdirp.sync(path.resolve(directory));
    fs.writeFile(path.resolve(path.join(directory, filename)), data, callback);
}
